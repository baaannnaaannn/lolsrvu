...
Options::Options(int argc, char** argv) {
    ...
    program.add_argument("protocol").help("Protocol version").default_value(std::string(protocol));
    program.add_argument("-p", "--player").help("Player name").default_value(std::string(PlayerName));
    program.add_argument("-c", "--champion").help("Champion name").default_value(std::string(ChampionName));
    program.add_argument("-s", "--skin")
        .help("Skin id")
        .action([](std::string const& v) { return std::stoi(v); })
    ...
    program.add_argument("--port")
        .help("Port")
        .action([](std::string const& v) { return (uint16_t)std::stoul(v); })
        .default_value((uint16_t)port);
    program.add_argument("--key").help("Encryption key").default_value(std::string(key));
