...
template <typename C, size_t S, typename T>
inline constexpr auto field(T&& value) {
    return Field<C, S, T>{value};
}
...
struct Data_in {
    ...
    inline constexpr Data_in(char const* data, size_t size) noexcept : start(data), cur(data), end(data + size) {}
